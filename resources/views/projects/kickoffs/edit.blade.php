<x-app-layout>
    <x-slot name="header">
        <div class="row mb-2 align-items-center">
            <div class="col-lg-8">
                <h1 class="m-0">{{ __('Edit Kick-off') }}</h1>
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('projects.kickoffs.update', $project) }}">
                @csrf
                @method('PUT')

                <div class="form-group">
                    <x-input-label for="purchase_order_number" :value="__('Purchase Order Number')" />
                    <x-text-input id="purchase_order_number" name="purchase_order_number" type="text" :value="old('purchase_order_number', $kickoff->purchase_order_number)" />
                ...
                <div class="form-group">
                    ...
                    <x-text-input id="scheduled_at" name="scheduled_at" type="text" data-datetimepicker="1" :value="old('scheduled_at', $kickoff->scheduled_at?->format('Y-m-d H:i'))" />
                    <x-input-error class="mt-2" :messages="$errors->get('scheduled_at')" />
                </div>

                <div class="form-group">
                    <x-input-label for="meeting_mode" :value="__('Meeting Mode')" />
                    <x-text-input id="meeting_mode" name="meeting_mode" type="text" :value="old('meeting_mode', $kickoff->meeting_mode)" />
                    <x-input-error class="mt-2" :messages="$errors->get('meeting_mode')" />
                </div>

                <div class="form-group">
                    <x-input-label for="stakeholders" :value="__('Stakeholders')" />
                    <textarea id="stakeholders" name="stakeholders" rows="3" class="form-control">{{ old('stakeholders', $kickoff->stakeholders) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea id="requirements_summary" name="requirements_summary" rows="3" class="form-control">{{ old('requirements_summary', $kickoff->requirements_summary) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" data-enhance="choices" class="form-control" required>
                        @foreach ($statuses as $status)
                            <option value="{{ $status }}" @selected(old('status', $kickoff->status) === $status)>
                                {{ __($status) }}
                            </option>
                        @endforeach
                    </select>
