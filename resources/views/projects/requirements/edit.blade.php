<x-app-layout>
    <x-slot name="header">
        <div class="row mb-2">
            <div class="col-sm-7">
                <h1 class="m-0">{{ __('Edit Requirement') }}</h1>
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('projects.requirements.update', [$project, $requirement]) }}">
                @csrf
                @method('PUT')

                <div class="form-group">
                    <x-input-label for="module_name" :value="__('Module Name')" />
                    <x-text-input id="module_name" name="module_name" type="text" :value="old('module_name', $requirement->module_name)" required autofocus />
                    <x-input-error class="mt-2" :messages="$errors->get('module_name')" />
                </div>

                <div class="form-group">
                    <x-input-label for="page_name" :value="__('Page Name')" />
                    <x-text-input id="page_name" name="page_name" type="text" :value="old('page_name', $requirement->page_name)" />
                    <x-input-error class="mt-2" :messages="$errors->get('page_name')" />
                </div>

                <div class="form-group">
                    <x-input-label for="title" :value="__('Title')" />
                    <x-text-input id="title" name="title" type="text" :value="old('title', $requirement->title)" required />
